...
Joueur* Partie::getJoueurActif() const { return joueur_actif; }

Joueur* Partie::getEnsenbleJoueurs() const { return joueurs; }

Joueur Partie::getJoueurI(const int i) { return joueurs[i]; }
...
Jeu* Partie::setJeu(Jeu* jeu) { this->jeu = jeu; }

Joueur* Partie::setJoueurActif(Joueur* joueur) { joueur_actif = joueur; }

Joueur* Partie::setEnsenbleJoueurs(Joueur* EnsembreJoueurs) {
  joueurs = EnsembreJoueurs;
}

Reserve* Partie::setReserve(Reserve* Reserve) { reserve = Reserve; }
...
void Partie::initialisation() {
  ...
  cout << "Entrez l'extention à laquelle vous voulez jouer (Marina, "
          "GreenValley, Deluxe) :"
       << endl;
  do {
  ...
  } while (lectureExtention != "MARINA" || lectureExtention != "GREENVALLEY" ||
           lectureExtention != "DELUXE");
...
void Partie::transaction_piece(Joueur& emeteur, Joueur& destinataire,
                               int montant) {
...
void Partie::transaction_carte(Joueur& emeteur, Joueur& destinataire,
                               Pile& pile) {
  // TODO: A completer
}
