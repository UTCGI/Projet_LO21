...
class Pile_Etablissement{
    ...
    void retirerCarte(unsigned int nb_cartes=1){
        if(nb_cartes>effectif) throw SetException("nombre de cartes a retirer trop eleve");
        else effectif-=nb_cartes;
    }
...
ostream& operator<<(ostream& f,Pile_Etablissement& pile){
 f<<"Pile de "<<pile.getEffectif()<<" exemplaire(s) de l'etablissement suivant :"<<endl<<endl<<pile.getEtablissement()<<endl;
