...
void Dialog::setmaxjoueur(){
    Jeu::libererInstance();
    Jeu::getInstance(Extensions[versions->checkedId()]);
    size_t cap = IA?Jeu::getInstance().getNb_joueurs_MAX()-1:Jeu::getInstance().getNb_joueurs_MAX();
...
void Dialog::addIA(){
    if (ajouterIA->text() == "Ajouter IA"){
        ...
        IA = true;
        while (static_cast<size_t>(form->rowCount()+1)>Jeu::getInstance().getNb_joueurs_MAX())
            form->removeRow(form->rowCount()-1);
    }
    else{
