...
Pile_Monument::Pile_Monument(const Monument* mo, unsigned int ef)
    : monument(mo), effectif(ef){};

// Methods
void Pile_Monument::retirerCarte(unsigned int nb_cartes) {
  if (nb_cartes > effectif)
    throw SetException("nombre de cartes a retirer trop eleve");
  else
    effectif -= nb_cartes;
}
void Pile_Monument::ajouterCarte(unsigned int nb_cartes) {
  effectif += nb_cartes;
}
